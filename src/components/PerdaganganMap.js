...
const PerdaganganMap = () => {
    ...
    const fetchShoppingData = async () => {
      ...
      try {
        ...
        if (result && result.datasets && Array.isArray(result.datasets)) {
          // Filter data where category is 'perdagangan'
          const filteredData = result.datasets.filter(row => row.category === 'perdagangan');
  ...
  const getPosition = (shopping) => {
    const lat = parseFloat(shopping.latitude);
    const lon = parseFloat(shopping.longitude);
    return [lat, lon];
  };
  ...
  return (
    ...
    >
      ...
      {shoppingData.map((shopping, index) => (
        <Marker key={index} position={getPosition(shopping)} icon={shoppingIcon}>
          <Popup>
            <div className="w-32">
              <div className="font-bold break-words">{shopping.name || 'Unknown Shop'}</div>
              <div className="inline-block p-2 mt-2 rounded text-right font-semibold" style={{ backgroundColor: 'rgba(238, 28, 67, 0.2)', color: '#EE1C43' }}>
                {categoryNames[shopping.category]}
              </div>
            </div>
          </Popup>
        </Marker>
