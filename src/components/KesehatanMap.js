...
const KesehatanMap = () => {
  ...
  const hospitalIcon = new Leaflet.Icon({
    iconUrl: '/hospitalIcon.svg',
    iconSize: [48,48],
    iconAnchor: [48/2, 48],
    ...
    const fetchHospitalData = async () => {
      ...
      try {
        ...
        if (result && result.datasets && Array.isArray(result.datasets)) {
          // Filter data where category is 'kesehatan'
          const filteredData = result.datasets.filter(row => row.category === 'kesehatan');
  ...
  return (
    ...
    >
      ...
      {hospitalData.map((hospital, index) => (
        <Marker key={index} position={getPosition(hospital)} icon={hospitalIcon}>
          <Popup>
            <div className="w-32">
              <div className="font-bold break-words">{hospital.name || 'Unknown Location'}</div>
              <div className="inline-block p-2 mt-2 rounded text-right font-semibold" style={{ backgroundColor: 'rgba(255, 139, 136, 0.2)', color: '#FF8B88' }}>
                {categoryNames[hospital.category]}
              </div>
            </div>
          </Popup>
        </Marker>
